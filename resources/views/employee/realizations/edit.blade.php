<x-layouts.employee title="Edit Realisasi Pajak" :header="'Edit Realisasi: ' . $realization->taxType->name . ' (' . $realization->year . ')'">
    ...
    <div class="max-w-4xl mx-auto">
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            <form action="{{ route('field-officer.realizations.update', $realization) }}" method="POST" class="p-6 space-y-8">
                @csrf
                @method('PUT')
                ...
                <div class="pt-6 border-t border-slate-100 flex items-center justify-end gap-3">
                    <a href="{{ route('field-officer.realizations.index') }}" class="px-4 py-2 text-sm font-medium text-slate-600 hover:text-slate-900 transition-colors">Batal</a>
