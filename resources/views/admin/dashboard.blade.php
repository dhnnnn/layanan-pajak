<x-layouts.admin title="Dashboard Admin" header="Dashboard Realisasi Pajak">
    ...
    <div class="space-y-6">
        @php
            ...
            $totalRealization = $dashboard->where('tax_type_parent_id', null)->sum('total_realization');
            $avgPercentage = $totalTarget > 0 ? ($totalRealization / $totalTarget) * 100 : 0;
            $totalMoreLess = $totalRealization - $totalTarget;
        @endphp
                        ...
                        <tr>
                            ...
                            @foreach(['q1', 'q2', 'q3', 'q4'] as $q)
                                <td class="px-3 py-4 border border-slate-300 text-right">
                                    {{ number_format($dashboard->where('tax_type_parent_id', null)->sum(fn($i) => $i['targets'][$q]), 0, ',', '.') }}
                                </td>
                                <td class="px-3 py-4 border border-slate-300 text-right">
                                    {{ number_format($dashboard->where('tax_type_parent_id', null)->sum(fn($i) => $i['realizations'][$q]), 0, ',', '.') }}
                                </td>
                                <td class="px-3 py-4 border border-slate-300 text-center">
                                    @php
                                        $t = $dashboard->where('tax_type_parent_id', null)->sum(fn($i) => $i['targets'][$q]);
                                        $r = $dashboard->where('tax_type_parent_id', null)->sum(fn($i) => $i['realizations'][$q]);
                                        $p = $t > 0 ? ($r / $t) * 100 : 0;
                                    @endphp
                                    {{ number_format($p, 0, ',', '.') }}
                                </td>
