<x-layouts.admin title="Edit User" header="Edit User">
    ...
    <div class="max-w-2xl mx-auto">
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
            <form action="{{ route('admin.rbac-users.update', $rbacUser) }}" method="POST" class="space-y-5">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $rbacUser->name) }}"
                    ...
                    <div>
                        ...
                        <input type="email" name="email" id="email" value="{{ old('email', $rbacUser->email) }}"
                    ...
                    <div>
                        <label for="password" class="block text-sm font-semibold text-slate-700 mb-1">Password Baru <span class="text-slate-400 font-normal">(opsional)</span></label>
                ...
                <div id="uptField" class="{{ in_array('kepala_upt', $roles->whereIn('id', old('roles', $assignedRoleIds))->pluck('name')->toArray()) ? '' : 'hidden' }}">
                    ...
                    <select name="upt_id" id="upt_id"
                        ...
                        @foreach($upts as $upt)
                            <option value="{{ $upt->id }}" @selected(old('upt_id', $assignedUptId) === $upt->id)>{{ $upt->name }}</option>
                        @endforeach
                    </select>
                    @error('upt_id')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
