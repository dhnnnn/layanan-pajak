<x-layouts.admin title="Import Target APBD" header="Import Target APBD">
    <div class="space-y-6">
        {{-- Upload Form --}}
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-6 overflow-hidden">
            <h3 class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-4">Unggah File Target APBD</h3>
            
            <form action="{{ route('admin.tax-targets.preview') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                @csrf
            ...
            <div class="mt-4 p-4 bg-blue-50/50 rounded-lg border border-blue-100">
                <div class="flex gap-3">
                    ...
                    <div class="text-xs text-blue-700 leading-relaxed">
                        <p class="font-bold mb-1">Petunjuk Import Target APBD:</p>
                        <ul class="list-disc ml-4 space-y-1">
                            <li>Download template "Target APBD" terlebih dahulu.</li>
                            <li>Kolom <strong>Jenis Pajak</strong> sudah terisi otomatis dari database.</li>
                            <li>Isi target APBD pada kolom yang tersedia.</li>
                            <li>Pastikan data sudah benar sebelum melakukan konfirmasi import.</li>
                        </ul>
                    </div>
                </div>
            </div>
        ...
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
            <div class="flex items-center justify-between">
                <div>
                    <h3 class="text-sm font-bold text-slate-800 mb-1">Lihat Laporan Target APBD</h3>
                    <p class="text-xs text-slate-600">Lihat rincian target APBD per jenis pajak</p>
                </div>
                <a href="{{ route('admin.tax-targets.report') }}" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold rounded-lg transition-colors shadow-sm inline-flex items-center gap-2">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"/>
                    </svg>
                    Lihat Laporan
                </a>
                <a href="{{ route('admin.tax-targets.manage') }}" class="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 text-sm font-semibold rounded-lg transition-colors shadow-sm inline-flex items-center gap-2">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
