<x-layouts.admin title="Edit Target APBD" :header="'Edit Target APBD: ' . $taxTarget->taxType->name . ' (' . $taxTarget->year . ')'">
    <x-slot:headerActions>
        <a href="{{ route('admin.tax-targets.manage', ['year' => $taxTarget->year]) }}" class="inline-flex items-center gap-2 px-4 py-2 text-slate-600 hover:text-slate-900 text-sm font-medium transition-colors">
            ...
            <form action="{{ route('admin.tax-targets.update', $taxTarget) }}" method="POST" class="p-6 space-y-5">
                ...
                <div>
                    ...
                    <select name="tax_type_id" id="tax_type_id" class="w-full rounded-lg bg-slate-50 text-slate-700 py-2.5 px-4 focus:bg-white focus:ring-2 focus:ring-blue-500/20 @error('tax_type_id') ring-2 ring-red-500/20 @endif" required>
                        @foreach($taxTypes as $type)
                            <option value="{{ $type->id }}" @selected(old('tax_type_id', (string) $taxTarget->tax_type_id) == (string) $type->id)>{{ $type->name }}</option>
                        @endforeach
                    </select>
                    @error('tax_type_id')
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="target_amount" id="target_amount" value="{{ old('target_amount', (int)$taxTarget->target_amount) }}" 
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="number" name="q1_target" id="q1_target" value="{{ old('q1_target', (int)$taxTarget->q1_target) }}" 
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <div class="absolute inset-y-0 left-0 pl-2.5 flex items-center pointer-events-none text-slate-400 text-[10px] font-bold">Rp</div>
                                <input type="number" name="q2_target" id="q2_target" value="{{ old('q2_target', (int)$taxTarget->q2_target) }}" 
                                    class="pl-8 w-full rounded-lg bg-slate-50 text-slate-600 text-sm py-2 px-3 focus:bg-white focus:ring-2 focus:ring-blue-500/10 border-transparent focus:border-blue-200">
                            </div>
                        </div>
                        <div>
                            ...
                            <div class="relative">
                                <div class="absolute inset-y-0 left-0 pl-2.5 flex items-center pointer-events-none text-slate-400 text-[10px] font-bold">Rp</div>
                                <input type="number" name="q3_target" id="q3_target" value="{{ old('q3_target', (int)$taxTarget->q3_target) }}" 
                                    class="pl-8 w-full rounded-lg bg-slate-50 text-slate-600 text-sm py-2 px-3 focus:bg-white focus:ring-2 focus:ring-blue-500/10 border-transparent focus:border-blue-200">
                            </div>
                        </div>
                        <div>
                            ...
                            <div class="relative">
                                <div class="absolute inset-y-0 left-0 pl-2.5 flex items-center pointer-events-none text-slate-400 text-[10px] font-bold">Rp</div>
                                <input type="number" name="q4_target" id="q4_target" value="{{ old('q4_target', (int)$taxTarget->q4_target) }}" 
                                    class="pl-8 w-full rounded-lg bg-slate-50 text-slate-600 text-sm py-2 px-3 focus:bg-white focus:ring-2 focus:ring-blue-500/10 border-transparent focus:border-blue-200">
                            </div>
                        </div>
                    </div>
                ...
                <div class="pt-4 border-t border-slate-100 flex items-center justify-end gap-3">
                    <a href="{{ route('admin.tax-targets.manage', ['year' => $taxTarget->year]) }}" class="px-4 py-2 text-sm font-medium text-slate-600 hover:text-slate-900 transition-colors">Batal</a>
