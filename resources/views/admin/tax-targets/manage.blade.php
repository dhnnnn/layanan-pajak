                            ...
                            <td class="px-4 py-3 text-center">
                                <div class="flex justify-center items-center gap-1.5">
                                    ...
                                    @if($item['tax_target_id'])
                                        <a href="{{ route('admin.tax-targets.edit', $item['tax_target_id']) }}" 
                                           class="p-1 text-amber-500 hover:text-amber-600 hover:bg-amber-50 rounded-lg transition-all" title="Edit Target / Anggaran">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    @else
                                        <a href="{{ route('admin.tax-targets.create', ['tax_type_id' => $item['tax_type_id'], 'year' => $year]) }}" 
                                           class="p-1 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-lg transition-all" title="Setel Override Target">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                            </svg>
                                        </a>
                                    ...
                                    {{-- Reset / Delete Action --}}
                                    @if($item['tax_target_id'])
                                        <form action="{{ route('admin.tax-targets.destroy', $item['tax_target_id']) }}" method="POST" class="inline" onsubmit="return confirm('Hapus override target ini dan kembali ke baseline Simpadu?')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="p-1 text-red-500 hover:text-red-700 hover:bg-red-50 rounded-lg transition-all" title="Reset (Kembali ke Anggaran Simpadu)">
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    @else
                                        <div class="p-1 text-slate-200 cursor-not-allowed" title="Sudah Menggunakan Target Baseline Simpadu">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                                            </svg>
                                        </div>
