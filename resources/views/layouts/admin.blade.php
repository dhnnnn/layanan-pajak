...
@if(auth()->user()->isAdmin() && !auth()->user()->isKepalaUpt())
            ...
            <a href="{{ route('pegawai.daily-entries.index') }}"
               class="flex items-center gap-3 px-3 py-2 rounded-lg transition-colors {{ request()->routeIs('pegawai.daily-entries.*') ? 'bg-blue-600 text-white' : 'text-slate-300 hover:bg-slate-700 hover:text-white' }}">
