...
use App\Actions\Tax\GetDailyRealizationAction;
use App\Actions\Tax\GetWeeklyRealizationAction;
...
class TaxTargetController extends Controller
{
     ...
     */
    public function dailyRealization(Request $request, GetDailyRealizationAction $getDailyRealization): JsonResponse
    {
        $year = (int) $request->query('year', date('Y'));

        return response()->json($getDailyRealization($year));
    }

    /**
     * Data realisasi bulan berjalan per minggu per ayat (AJAX).
     */
    public function weeklyRealization(Request $request, GetWeeklyRealizationAction $getWeeklyRealization): JsonResponse
    {
        $year = (int) $request->query('year', date('Y'));
        $month = (int) $request->query('month', date('n'));

        return response()->json($getWeeklyRealization($year, $month));
    }
}
